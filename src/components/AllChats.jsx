    ...
    return (
        <>
            <div className='md:mx-2 h-[95%] ' >
                ...
                <div className='bg-[#6e36a5] p-5 h-[90%] overflow-auto  rounded-xl'>
                    {users.map((user, i) => (
                        <div onClick={() => { user?.users[0]?._id === userDetails._id ? handleOpenChat(user?.users[1]) : handleOpenChat(user?.users[0]) }} key={i} className={` flex justify-center items-center ${user?.users[0]._id === userDetails?._id ? user?.users[1]?._id === currentUser?._id && "bg-[#936bbb]" : user?.users[0]?._id === currentUser?._id && "bg-[#936bbb]"} transition-all  rounded-xl  pe-6 ps-6 mb-3`}>
                            <img src='/profile-icon.jpg' alt='user' className='border  w-10 mx-3 my-1 h-10 me-3 rounded-full' />
                            <div className='flex justify-center items-center '>
                                ...
                                <div className='ms-10  text-end'>
                                    <h1 className='text-sm font-bold'>{ user?.users[0]?._id === userDetails._id ? user?.users[1]?.lastOnline  : user?.users[0]?.lastOnline}</h1>
